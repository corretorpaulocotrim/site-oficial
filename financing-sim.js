...
/* ---------- MOTOR DE CRÉDITO CAIXA / MCMV 2026 (RJ · Sudeste) ----------
   Base: regras vigentes MCMV 2026 (abril/2026). Refinar com a tabela interna
   do Paulo (Portal de Relacionamento Caixa) editando FSIM_TAXAS abaixo.
   aa = juros ao ano não-cotista FGTS · aaCot = cotista FGTS (-0,5 p.p.)
   sub = teto de subsídio estimado no topo da faixa (cai conforme a renda sobe). */
var FSIM_TAXAS = [
  {max:3200,      faixa:'Faixa 1', aa:5.00,  aaCot:4.50,  sub:55000, tetoImovel:275000},
  {max:5000,      faixa:'Faixa 2', aa:6.00,  aaCot:5.50,  sub:25000, tetoImovel:275000},
  {max:9600,      faixa:'Faixa 3', aa:8.00,  aaCot:7.50,  sub:0,     tetoImovel:400000},
  {max:13000,     faixa:'Faixa 4', aa:10.50, aaCot:10.00, sub:0,     tetoImovel:600000},
  {max:Infinity,  faixa:'SBPE',    aa:11.79, aaCot:11.79, sub:0,     tetoImovel:Infinity}
];
var FSIM_PRAZO_MESES = 420;      // 35 anos (MCMV 2026)
var FSIM_DTI = 0.30;             // comprometimento máx. da renda com a prestação
var FSIM_PCT_FIN_MAX = 0.80;     // Caixa financia até 80% do valor (SAC)

function fsimFaixaDe(renda){
  for(var i=0;i<FSIM_TAXAS.length;i++){ if(renda<=FSIM_TAXAS[i].max) return FSIM_TAXAS[i]; }
  return FSIM_TAXAS[FSIM_TAXAS.length-1];
}
/* Análise de crédito real: quanto a renda libera, parcela real, subsídio, veredito. */
function fsimAnaliseCaixa(renda, preco, cotista){
  var f = fsimFaixaDe(renda);
  var aa = cotista ? f.aaCot : f.aa;
  var i = aa/100/12;                        // taxa mensal nominal
  var n = FSIM_PRAZO_MESES;
  var fator = i>0 ? (1-Math.pow(1+i,-n))/i : n;   // fator Price
  var pmtMax = renda*FSIM_DTI;              // maior prestação que a renda suporta
  var financiaMaxRenda = pmtMax*fator;      // quanto a renda libera de financiamento
  var financiaMaxValor = preco*FSIM_PCT_FIN_MAX;
  var financiado = Math.max(0, Math.min(financiaMaxRenda, financiaMaxValor));
  var parcela = i>0 ? financiado*i/(1-Math.pow(1+i,-n)) : financiado/n;
  // subsídio estimado: decresce linearmente dentro da faixa
  var pisoFaixa = (FSIM_TAXAS.indexOf(f)>0) ? FSIM_TAXAS[FSIM_TAXAS.indexOf(f)-1].max : 0;
  var prog = f.max===Infinity ? 1 : (f.max>pisoFaixa ? (renda-pisoFaixa)/(f.max-pisoFaixa) : 1);
  var subsidio = Math.max(0, Math.round(f.sub*(1-prog)/100)*100);
  var rendaCobre = financiaMaxRenda >= financiaMaxValor - 1;   // renda banca 80% do imóvel?
  var entradaMin = Math.max(0, preco - financiado - subsidio);
  return {
    faixa:f.faixa, aa:aa, cotista:!!cotista, prazo:n,
    financiaMaxRenda:financiaMaxRenda, financiado:financiado, parcela:parcela,
    subsidio:subsidio, rendaCobre:rendaCobre, entradaMin:entradaMin,
    tetoImovel:f.tetoImovel, acimaTeto:(f.tetoImovel!==Infinity && preco>f.tetoImovel)
  };
}
...
var FSIM_LAST = null, FSIM_LAST_CAIXA = null, FSIM_NOME='', FSIM_CONSTRUTORA='', FSIM_BAIRRO='', FSIM_APRESENTACAO='';
...
function fsimUpdate(){
  ...
  // --- Análise de crédito Caixa (motor 2026) ---
  var caixaEl = document.getElementById('fsimCaixa');
  if(caixaEl){
    ...
    var grid = '<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px">'
      ...
      + cell('Prazo', an.prazo+' meses')
      + '</div>'
      + '<div style="display:flex;flex-wrap:wrap;gap:10px">'
      + cell('Sua renda libera até', fmtBRL(an.financiaMaxRenda))
      + cell('Prestação do financiamento', fmtBRL(an.parcela)+'/mês', true)
